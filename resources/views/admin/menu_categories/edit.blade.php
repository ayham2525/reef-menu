{{-- resources/views/admin/menu_categories/edit.blade.php --}}
@extends('adminlte::page')

@section('title', __('Menu Categories') . ' - ' . __('Edit'))

@section('content_header')
<div class="d-flex justify-content-between align-items-center">
    <h1 class="mb-0">
        <i class="fa fa-edit text-primary mr-2"></i>{{ __('Edit Category') }} ({{ $category->name }})
    </h1>
...
<form action="{{ route('admin.menu-categories.update', $category) }}" method="POST" autocomplete="off">
    @csrf @method('PUT')
    ...
    <div class="card shadow-sm">
        ...
        <div class="card-body">
            <div class="row">
                ...
                <div class="col-md-3">
                    <label>{{ __('Code') }}</label>
                    <input type="text" name="code" value="{{ old('code', $category->code) }}" class="form-control @error('code') is-invalid @enderror">
                    @error('code')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>

                <div class="col-md-3">
                    <label>{{ __('Sort Order') }}</label>
                    <input type="number" name="sort_order" value="{{ old('sort_order', $category->sort_order ?? 0) }}" min="0" step="1" class="form-control @error('sort_order') is-invalid @enderror">
                ...
                <div class="col-md-6 mt-3">
                    ...
                    <div class="input-group">
                        <input type="text" name="slug" id="slug" value="{{ old('slug', $category->slug) }}" class="form-control @error('slug') is-invalid @enderror" readonly>
        ...
        <div class="card-footer d-flex justify-content-between">
            <a href="{{ route('admin.menu-categories.show', $category) }}" class="btn btn-outline-secondary">
...
<script>
(function(){
    function slugify(str){ return (str||'').toString().normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-+|-+$/g,'').replace(/--+/g,'-');}
