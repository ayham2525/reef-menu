...
@section('title', __('Positions') . ' - ' . __('Create'))

@section('content_header')
    <div class="d-flex justify-content-between align-items-center">
        <h1 class="mb-0">
            <i class="fa fa-briefcase text-primary mr-2"></i>{{ __('New Position') }}
        </h1>
...
@section('content')
    ...
    <form action="{{ route('admin.positions.store') }}" method="POST" autocomplete="off">
        @csrf

        <div class="card shadow-sm">
            <div class="card-header">
                <h3 class="card-title mb-0">
                    <i class="fa fa-plus-circle mr-2 text-primary"></i>{{ __('Create Position') }}
                </h3>
            </div>
                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <small class="form-text text-muted">
                                {{ __('The human-readable title of the position.') }}
                            </small>
                        </div>
                    </div>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <small class="form-text text-muted">
                                {{ __('Short identifier (e.g., RM, MG).') }}
                            </small>
                        </div>
                    </div>

                    {{-- Slug (auto from name, toggle editable) --}}
                    <div class="col-md-6">
                        <div class="form-group">
                            <label for="slug">
                            ...
                            <small class="form-text text-muted">
                                {{ __('Auto-generated; click the lock to edit manually if needed.') }}
                            </small>
                        </div>
                    </div>
                    ...
                    <div class="col-md-12 mt-3">
                        <div class="form-group">
                            ...
                            <textarea
                                ...
                                placeholder="{{ __('Optional notes or details') }}">{{ old('description') }}</textarea>
                            @error('description')
                                <div class="invalid-feedback d-block">{{ $message }}</div>
                            @enderror
                        </div>
            ...
            <div class="card-footer d-flex justify-content-between">
                ...
                <button type="submit" class="btn btn-primary custom-btn">
                    <i class="fa fa-save mr-1"></i>{{ __('Save Position') }}
                </button>
            </div>
        </div>
    </form>
...
<script>
    ...
    (function () {
        ...
        // Auto-generate slug from name while not in manual mode
        ...
        // Toggle readonly/manual mode
        toggleBtn.addEventListener('click', function () {
            manualSlug = !manualSlug;
            slugInput.readOnly = !manualSlug;
            this.innerHTML = manualSlug
                ? '<i class="fa fa-unlock"></i>'
                : '<i class="fa fa-lock"></i>';
            this.title = manualSlug ? '{{ __('Lock slug') }}' : '{{ __('Edit slug') }}';
            slugInput.focus();
        });

        // If user types in slug, switch to manual mode implicitly
        slugInput.addEventListener('input', function () {
            manualSlug = true;
            toggleBtn.innerHTML = '<i class="fa fa-unlock"></i>';
            toggleBtn.title = '{{ __('Lock slug') }}';
        });

        // Initialize default slug if empty and name has value
