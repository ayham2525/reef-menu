...
@section('content_header')
    <div class="d-flex justify-content-between align-items-center">
        <h1 class="mb-0">
            <i class="fa fa-briefcase text-primary"></i> {{ __('Positions') }}
        </h1>
   ...
   @endif
    ...
    <ul class="nav nav-pills mb-3" id="status-pills">
        <li class="nav-item custom-nav-item">
            <a href="{{ route('admin.positions.index', array_filter(['search' => request('search'), 'status' => 'all'])) }}"
        ...
        <li class="nav-item custom-nav-item">
            <a href="{{ route('admin.positions.index', array_filter(['search' => request('search'), 'status' => 'active'])) }}"
        ...
        <li class="nav-item custom-nav-item">
            <a href="{{ route('admin.positions.index', array_filter(['search' => request('search'), 'status' => 'inactive'])) }}"
    ...
    <div class="mb-3" style="max-width: 480px;">
        <div class="input-group">
            <input type="text" id="position-search" name="search" value="{{ request('search') }}" class="form-control"
...
(function () {
    ...
    function makeUrl(pageUrl = null) {
        const base = pageUrl ? new URL(pageUrl, window.location.origin)
                             : new URL("{{ route('admin.positions.index') }}", window.location.origin);
        ...
        pills.querySelectorAll('a.nav-link').forEach(a => {
            a.addEventListener('click', function (ev) {
                ...
                pills.querySelectorAll('a.nav-link').forEach(el => {
                    el.classList.remove('custom-btn', 'active');
                });
