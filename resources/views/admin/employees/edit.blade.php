...
@section('title', __('Employees') . ' - ' . __('Edit'))

@section('content_header')
<div class="d-flex justify-content-between align-items-center">
    <h1 class="mb-0">
        <i class="fa fa-user-edit text-primary mr-2"></i>{{ __('Edit Employee') }} ({{ $employee->employee_code }})
    </h1>
...
<form action="{{ route('admin.employees.update', $employee) }}" method="POST" autocomplete="off">
    @csrf @method('PUT')
        ...
        <div class="card-body">
            <div class="row">
                <div class="col-md-6">
                    ...
                    <input type="text" name="user_name" value="{{ old('user_name', $employee->user->name) }}"
                           class="form-control @error('user_name') is-invalid @enderror" required>
                    @error('user_name')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>
                <div class="col-md-6">
                    <label>{{ __('Email') }} <span class="text-danger">*</span></label>
                    <input type="email" name="user_email" value="{{ old('user_email', $employee->user->email) }}"
        ...
        <div class="card-body">
            <div class="row">
                ...
                <div class="col-md-6 mt-3">
                    ...
                    <input type="text" name="employee_code" value="{{ old('employee_code', $employee->employee_code) }}" class="form-control @error('employee_code') is-invalid @enderror">
                ...
                <div class="col-md-6 mt-3">
                    ...
                    <input type="text" name="national_id" value="{{ old('national_id', $employee->national_id) }}" class="form-control @error('national_id') is-invalid @enderror">
                ...
                <div class="col-md-6 mt-3">
                    ...
                    <input type="date" name="birth_date" value="{{ old('birth_date', optional($employee->birth_date)->format('Y-m-d')) }}" class="form-control @error('birth_date') is-invalid @enderror">
                    @error('birth_date')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>

                <div class="col-md-6 mt-3">
                    <label>{{ __('Hired At') }}</label>
                    <input type="date" name="hired_at" value="{{ old('hired_at', optional($employee->hired_at)->format('Y-m-d')) }}" class="form-control @error('hired_at') is-invalid @enderror">
                    @error('hired_at')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>

                <div class="col-md-6 mt-3">
                    <label>{{ __('Terminated At') }}</label>
                    <input type="date" name="terminated_at" value="{{ old('terminated_at', optional($employee->terminated_at)->format('Y-m-d')) }}" class="form-control @error('terminated_at') is-invalid @enderror">
                ...
                <div class="col-md-12 mt-3">
                    ...
                    <textarea name="notes" rows="4" class="form-control @error('notes') is-invalid @enderror">{{ old('notes', $employee->notes) }}</textarea>
