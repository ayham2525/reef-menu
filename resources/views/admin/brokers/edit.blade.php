...
@section('content_header')
    <div class="d-flex justify-content-between align-items-center">
        <h1 class="mb-0">
            <i class="fa fa-user-tie text-primary"></i> {{ __('Edit Broker') }}
        </h1>
...
@section('content')
    ...
    <form action="{{ route('admin.brokers.update', $broker) }}" method="POST" autocomplete="off">
        @csrf
        @method('PUT')
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <select name="agency_id" id="agency_id" class="form-control @error('agency_id') is-invalid @enderror">
                            ...
                            @foreach($agencies as $agency)
                                <option value="{{ $agency->id }}" {{ old('agency_id', $broker->agency_id) == $agency->id ? 'selected' : '' }}>
                                    {{ $agency->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="form-row">
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <select id="is_active" name="is_active" class="form-control @error('is_active') is-invalid @enderror">
                            <option value="1" @selected(old('is_active', (int)$broker->is_active) == 1)>{{ __('Active') }}</option>
                            <option value="0" @selected(old('is_active', (int)$broker->is_active) == 0)>{{ __('Inactive') }}</option>
                        </select>
